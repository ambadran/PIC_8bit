...
void init_I2C_Master(long long int baudRate) {
...
unsigned char sendI2C(unsigned char address, unsigned char data) {
    start_condition();
...
unsigned char rawRead_oneByte(void) {
    unsigned char Data;
    wait();
    RCEN = 1;
    wait();
...
unsigned char readI2C(unsigned char address) {
    start_condition();
    rawSend(address + 1);
    unsigned char result = rawRead_oneByte();
    stop_condition();
    return result;
}

void init_I2C_Slave(int address) {
    TRISB0 = 1; // SDA pin input
    TRISB1 = 1; // SCL pin input
